...
class Grid extends Component {
        ...
        if (!enableVisualizeButton && !this.state.routing) {
            this.setState({ routing: true }, () => {
                switch (currentAlg) {
                    ...
                    case 4:
                        this.bidirectionalDijkstra(gridCells, playerPos, targetPos, markVisited, markSP, findPath);
                        break;

                    case 5:
                        this.bidirectionalAStar(gridCells, playerPos, targetPos, markVisited, markSP, findPath);
                        break;
    ...
    bidirectionalDijkstra = async (gridCells, playerPos, targetPos, markVisited, markSP, findPath) => {
        let { graph, cellIdPositionMap } = this.initializeGraph(gridCells);
        let playerId = gridCells[playerPos.i][playerPos.j].id;
        let targetId = gridCells[targetPos.i][targetPos.j].id;
        ...
        while (!pqForward.isEmpty() && !pqBackward.isEmpty()) {
            ...
            if (currentBackward.id === targetId) {
                ...
                shortestDistanceBackward.set(currentBackward.id, currentBackward.weight);
            }

            // mu is the true distance s-t
            if (
                shortestDistanceForward.get(currentForward.id) + shortestDistanceBackward.get(currentBackward.id) >=
                mu
            ) {
                this.drawShortestPath(parentForward, playerId, intersectingNode, cellIdPositionMap, markSP, true);
                this.drawShortestPath(parentBackward, targetId, intersectingNode, cellIdPositionMap, markSP, true);
                break;
            }
            ...
            while (headForward !== null) {
                let totalWeight = shortestDistanceForward.get(currentForward.id) + headForward.weight;

                if (pqForward.containsKey(headForward.id) && pqForward.peek(headForward.id).weight > totalWeight) {
                ...
                if (
                    ...
                    shortestDistanceForward.get(currentForward.id) + w + shortestDistanceBackward.get(headForward.id) <
                        mu
                ) {
                    let w =
                        Math.abs(currentForward.i - currentBackward.i) + Math.abs(currentForward.j - currentBackward.j);
            ...
            while (headBackward !== null) {
                let totalWeight = shortestDistanceBackward.get(currentBackward.id) + headBackward.weight;

                if (pqBackward.containsKey(headBackward.id) && pqBackward.peek(headBackward.id).weight > totalWeight) {
                ...
                let { i, j } = cellIdPositionMap.get(headBackward.id);
                let w = Math.abs(currentBackward.i - i) + Math.abs(currentBackward.j - j);
                if (
    ...
    bidirectionalAStar = async (gridCells, playerPos, targetPos, markVisited, markSP, findPath) => {
        let { graph, cellIdPositionMap } = this.initializeGraph(gridCells);
        let playerId = gridCells[playerPos.i][playerPos.j].id;
        let targetId = gridCells[targetPos.i][targetPos.j].id;
        ...
        while (!pqForward.isEmpty() && !pqBackward.isEmpty()) {
            ...
            while (headForward !== null) {
                let g = shortestDistanceForward.get(currentForward.id) + headForward.weight;

                let { i, j } = cellIdPositionMap.get(headForward.id);
                let h = Math.abs(targetPos.i - i) + Math.abs(targetPos.j - j);

                let f = g + h * 1.001;
                if (pqForward.containsKey(headForward.id) && pqForward.peek(headForward.id).weight > f) {
            ...
            while (headBackward !== null) {
                let g = shortestDistanceBackward.get(currentBackward.id) + headBackward.weight;
                ...
                if (pqBackward.containsKey(headBackward.id) && pqBackward.peek(headBackward.id).weight > f) {
    ...
    drawShortestPath = async (parent, playerId, targetId, cellIdPositionMap, markSP, bidirectional = false) => {
        if (bidirectional) {
            let temp = targetId;
                        ...
                        <tr key={i}>
                            {row.map((col, j) => (
                                <GridCell
                                    ...
                                    onCellClicked={(i, j) => cellClick(i, j, algorithms[currentAlg].type)}
                                    onDragDrop={dragNDrop}
                                    mouseDown={onMouseDown}
                                    mouseOver={(i, j) => onMouseOver(i, j, algorithms[currentAlg].type)}
                                ></GridCell>
                            ))}
                        </tr>
                    ))}
